...
class Agent extends Model
{
     ...
     * 根据父级ID查询出全部子级
     */
    public static function recursion($agent_id)
    {
        if (Cache::has("child_agent_ids:{$agent_id}")) {
            $child_agents = Cache::get("child_agent_ids:{$agent_id}");
        } else {
            $childOne =  DB::table('agent')->where('parent_agent_id', $agent_id)->get()->map(function ($value) {return (array)$value;})->toArray();
            $child_agents = self::getChildIds($childOne);
            Cache::put("child_agent_ids:{$agent_id}", $child_agents, Carbon::now()->addMinutes(5));
        }
    ...
    public static function getChildIds($res)
    {
        $output = array();
        foreach ($res as $k => $v)
        {
            $tmpRes = DB::table('agent')->where('parent_agent_id', $v['id'])->get()->map(function ($value) {return (array)$value;})->toArray();

            array_push($output,$v['id']);
            if (!empty($tmpRes))
            {
                $output = array_merge($output, self::getChildIds($tmpRes));
            }
        }
        return $output;
    }
