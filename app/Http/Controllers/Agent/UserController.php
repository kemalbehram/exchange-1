...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        //获取所有代理商
        $agent = DB::table('agent')->where('status', 1)->select('username', 'id')->get()->toArray();

        return view("agent.user.index", ['parent_id' => $parent_id, 'legal_currencies' => $legal_currencies, 'agent' => $agent, 'is_admin' => $_self->is_admin]);
    }
    ...
    public function lists(Request $request)
    {
        ...
        $users = $users->orWhereRaw("FIND_IN_SET($agent_id,users.agent_path)");

        //根据父级ID查询出全部子级
        $childAll = Agent::recursion($agent_id);
        foreach ($childAll as $k=>$v){
            $users = $users->orWhereRaw("FIND_IN_SET($v,users.agent_path)");
        }
     ...
     * @param Request $r
     */
    public function choise_agent(Request $request)
    {
        ...
        if(!$agent_id){
            return $this->error('请选择代理商');
        }

        $users = new Users();
        $info = $users->where('id',$user_id)->first();
        $res = $users->where('id', $user_id)->update(['agent_path' => $info->agent_path.','.$agent_id,'agent_note_id' => $agent_id]);
