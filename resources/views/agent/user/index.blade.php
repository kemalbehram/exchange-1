                 ...
                 <div class="layui-col-md10" style="margin-top:30px">
                         <form class="layui-form" id="endit" lay-filter="example">
                                 <div class="layui-form-item">
                                         ...
                                         <div class="layui-input-block">
                                                 <select name="sale" lay-filter="sale" id="sale">
                                                         <option value="0">请选择</option>
                                                         @foreach ($agent as $v)
                                                             <option value="{{$v->id}}" class="ww">{{$v->username}}</option>
                                                         @endforeach
                                                 </select>
                                             </div>
                                 </div>
...
@section('scripts')
    <script type="text/html" id="table-useradmin-webuser">
        @if($is_admin== 1 )
        <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="choise_sale">选择代理商</a>
        @endif
                 ...
                 layer.open({
                     ...
                     ,btn:['提交','取消'],
                         yes:function(index,layero){
                             var user_id = data.id
                             var agent_id = $('#sale').val()
                             admin.req({
                                 type : "POST",
                                 url : '/agent/user/choise_agent',
                                 dataType : "json",
